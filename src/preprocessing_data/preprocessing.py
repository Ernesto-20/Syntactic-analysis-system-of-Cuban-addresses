...
class PreprocessingData(ABC):
    ...
    def adapt_data_set(self, data_set: DataFrame):
        ...
        n_tag = len(id_to_category) + 1  # more padding
        ...
        # write = ExcelWriter('Prueba.xlsx')
        # group_addres.to_excel(write, 'PRUEBA', index = False)
        # write.save()
        ...
        max_len_characters = max([len(''.join(s)) for s in group_addres['Word'].tolist()]) # Se cuenta tambien los signos de puntacion.
        ...
        return DataSet(vocabulary_word, max_len_characters, max_len_trigram, max_len_word, n_tag, id_to_category, input_dim,
