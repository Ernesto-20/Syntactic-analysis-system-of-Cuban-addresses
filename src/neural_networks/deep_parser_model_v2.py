...
class DeepParserConfig:
    def __init(self, output_emb_char,
        ...
        self.__dropout_trigram_trigram = dropout_trigram_trigram
    ...
    def get_dropout_trigram_trigram(self):
        return self.__dropout_trigram_trigram
...
class DeepParserModel(NeuralParser):
    ...
    def __init__(self, data_set: DataSet, cleaner_method, config: DeepParserConfig, model=None):
        self.data = data_set
        ...
        if model is None:
            ...
            embedding_character = Embedding(vocab_size_character, 100, name='Character_Embedding')
            layer_embedding_character = embedding_character(layer_tv_character)
            blstm_character = Bidirectional(LSTM(units=50, return_sequences=True, dropout=0, recurrent_dropout=0),
                                            merge_mode='concat', name='Character_BLSTM')
            layer_blstm_character = blstm_character(layer_embedding_character)

            embedding_trigram = Embedding(vocab_size_trigram, 100, name='Trigram_Embedding')
            layer_embedding_trigram = embedding_trigram(layer_tv_by_trigram)
            blstm_trigram = Bidirectional(LSTM(units=56, return_sequences=True, dropout=0, recurrent_dropout=0),
                                          merge_mode='concat', name='Trigram_BLSTM')
            layer_blstm_trigram = blstm_trigram(layer_embedding_trigram)

            embedding_word = Embedding(vocab_size_word, 128, name='Word_Embedding')
            ...
            blstm_concat = Bidirectional(
                LSTM(units=data_set.get_max_len_word(), return_sequences=False, dropout=0.1, recurrent_dropout=0),
                merge_mode='sum', name='Character_Trigram_BLSTM')
            layer_blstm_concat = blstm_concat(concat)

            # ********* PROJECTION LAYER ***************
            projection = Reshape((data_set.get_max_len_word(), 1), name='Character_Trigram_Projection')(layer_blstm_concat)
            concat_2 = Concatenate(name='Word_Projection_Concat')([projection, layer_embedding_word])
            # ********* PROJECTION LAYER ***************


            blstm_concat_2 = Bidirectional(
                LSTM(units=data_set.get_n_tag(), return_sequences=True, dropout=0.1, recurrent_dropout=0),
