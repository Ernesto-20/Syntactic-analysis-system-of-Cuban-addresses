...
class DeepParserModel(NeuralParser):
    ...
    def __init__(self, data_set: DataSet, cleaner_method, model=None):
        ...
        if model is None:
            ...
            blstm_character = Bidirectional(LSTM(units=16, return_sequences=True, dropout=0.5, recurrent_dropout=0),
                                            merge_mode='concat')
            ...
            blstm_trigram = Bidirectional(LSTM(units=16, return_sequences=True, dropout=0.5, recurrent_dropout=0),
                                          merge_mode='concat')
            layer_blstm_trigram = blstm_trigram(layer_embedding_trigram)

            embedding_word = Embedding(vocab_size_word, 128, name='Embedding_Word')
