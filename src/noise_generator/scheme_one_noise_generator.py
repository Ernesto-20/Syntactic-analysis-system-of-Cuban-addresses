...
from src.noise_generator.noise_generator import NoiseGenerator
from src.utils.lookup import STREET_NAME_PREFIX, STREET_NAME_SUFFIX, LOCALITY_PREFIX, MUNICIPALITY_PREFIX, \
...
class SchemeOneNoiseGenerator(NoiseGenerator):
    '''
        Caracterizacion del modelo 1: (Componentes para permutar)

        De acuerdo a la localidad, municipio, provincia se tiene los siguientes componentes: (Componentes DPA)
    ...
    def generate_noise(self, data_set: DataFrame, address_amount=None):
        ...
        for i in data_set.index:
            ...
            if not self._is_empty(first_side_street) and not self._is_empty(second_side_street):
                # Is type one
                principal_street_added_word = self._generate_prefix_or_suffix_randomly(principal_street,
                                                                                        entity_type='principal_street')
                first_side_street_added_word = self._generate_prefix_or_suffix_randomly(first_side_street,
                                                                                         entity_type='first_side_street')
                second_side_street_added_word = self._generate_prefix_or_suffix_randomly(second_side_street,
                ...
                conjunction_prefix = [
                    ['e' if (second_side_street_added_word[0][0][0].lower() == 'i') else self._generate_conjunction(), 'rw']]
            ...
            elif not self._is_empty(first_side_street) or not self._is_empty(second_side_street):
                #   Is type 2
                side_street = first_side_street if self._is_empty(second_side_street) else second_side_street

                principal_street_added_word = self._generate_prefix_or_suffix_randomly(principal_street,
                                                                                        entity_type='principal_street')
                side_street_added_word = self._generate_prefix_or_suffix_randomly(side_street,
                                                                                   entity_type='first_side_street')
                ...
                if rm.randint(1, 100) <= 50:
                    ...
                    conjunction_prefix = [
                        ['e' if side_street_added_word[0][0][0] == 'i' else self._generate_conjunction(), 'rw']]
            ...
            # Components DPA
            if not self._is_empty(locality):
            ...
            if not self._is_empty(municipality):
            ...
            if not self._is_empty(province):
            ...
            address_number += 1
            self._add_new_address(components, address_number, address_list, words_list, tags_list)

        # Adding real address
        real_address = self._add_real_address()
        for address in real_address:
            address_number += 1
            self._add_new_address(address, address_number, address_list, words_list, tags_list)
        print('El total de direcciones fue de ', address_number)
        return self._generate_data_frame(address_list, words_list, tags_list)
    ...
    def generate_building_and_apartment(self):
        ...
        identification_building = self._generate_building_syntetic()
        ...
        if random_value > 40:
            ...
            identification_apartment = self._generate_apartment_syntetic()
    ...
    def _generate_data_frame(self, address_list, words_list, tags_list):
        return DataFrame({
    ...
    def _add_new_address(self, components, address_number, address_list, words_list, tags_list):
    ...
    def _generate_apartment_syntetic(self):
        random_value = rm.randint(1, 100)
        if random_value <= 50:
            # only numbers
    ...
    def _generate_prefix_or_suffix_randomly(self, street_name: str, entity_type: str):
        '''
        :param street_name:
        :return:
        ...
        if not street_name_contain_some_type_prefix:
            possibility_suffix = self._eval_possibility_suffix_street(street_name)
    ...
    def _eval_possibility_suffix_street(self, street_name: str):

        if not self._is_empty(street_name):
    ...
    def _generate_conjunction(self):
        '''
            This function will return one of the following conjunctions "and, &, &&"
        '''
        if rm.randint(1, 100) <= 6:
            if rm.randint(1, 10) < 7:
                return '&'
            else:
                return '&%'
        else:
