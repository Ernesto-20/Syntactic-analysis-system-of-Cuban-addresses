...
print('EXP6_C5_D100K')
model_type_two = NeuralParserManage.load_neural_parser(route='../../assets/trained_models/model_type_two/',
                                                                                       name='EXP6_C5_D100K')
...
main_decoder = Decoder(model_type_two.data.get_id_to_category(),
                       AddressCleaner.cleaner_method('custom_standardization_v2'))

main_parser = AddressParser(model_type_two, main_decoder)

lista = pd.read_excel('lista_eval_schema_building.xlsx')

result_list = main_parser.process_address_two(lista['full_address'].to_list())


lista_real = import_address_list_two(lista)
...
print(calculate_results_two(result_list, lista_real))